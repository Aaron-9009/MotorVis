...
{
    (void) rx_bytes_since_log;
    (void) latest_sentence;

    // TODO: Testing Code
    // Parser status logging is preserved for future UART debugging, but hidden during the coordinate-only packet test.
    ...
    while (true) {
        ...
        if (s_gps.location.isUpdated()) {
            ...
            if (motor_vis_gnss_store_fix(fix_valid, &stored_fix) && fix_valid) {
                ...
                // );
                // TODO: Testing Code
                // Previous diagnostic log showed the stored packet fields before the demo narrowed output to decoded lat/long only.
                // ESP_LOGI(
                //     TAG,
                ...
                // TODO: Testing Code
                // Invalid-fix diagnostics are preserved for future hardware troubleshooting, but hidden for the coordinate-only packet test.
                // char utc_time_text[16] = {0};
                // char hdop_text[16] = {0};
                // uint32_t satellites = s_gps.satellites.isValid() ? s_gps.satellites.value() : 0U;
