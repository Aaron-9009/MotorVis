...
/* Local MotoVis build patch:
 * ESP-IDF 5.5 keeps freertos/task_snapshot.h as a deprecated compatibility
 * header that emits a #warning, and this project currently treats warnings as
 * errors. Include the real header directly so the diagnostics component still
 * gets TaskSnapshot_t and uxTaskGetSnapshotAll() without failing the build.
 */
